...
<div class="toolbar" role="toolbar" aria-label="Outils de dessin">
  {#each tools as tool}
    <button
      ...
      onclick={() => editorState.activeTool = tool.id}
      title="{tool.label} ({tool.shortcut})"
      aria-pressed={editorState.activeTool === tool.id}
    >
      <svg width="20" height="20" viewBox="0 0 24 24" fill="currentColor">
        {#if tool.id === 'line'}
          <path d={tool.icon} stroke="currentColor" stroke-width="2" fill="none" stroke-linecap="round" />
        {:else}
          <path d={tool.icon} />
        {/if}
      </svg>
  ...
  <button
    class="tool-btn"
    class:active={editorState.mirrorMode}
    onclick={() => editorState.mirrorMode = !editorState.mirrorMode}
    title="Miroir (M)"
    aria-pressed={editorState.mirrorMode}
  >
    <svg width="20" height="20" viewBox="0 0 24 24" fill="currentColor">
      <path d="M12 2v20M9 6l-6 6 6 6V6Zm6 0v12l6-6-6-6Z" />
    </svg>
  ...
  {#if editorState.activeTool === 'rect'}
    <button
      class="tool-btn tool-btn--sub"
      onclick={() => editorState.rectFillMode = editorState.rectFillMode === 'filled' ? 'outline' : 'filled'}
      title={editorState.rectFillMode === 'filled' ? 'Mode: Rempli' : 'Mode: Contour'}
    >
      <svg width="18" height="18" viewBox="0 0 24 24">
        {#if editorState.rectFillMode === 'filled'}
...
<style>
  ...
  .tool-btn:focus-visible { outline: 2px solid rgba(94, 144, 255, 0.5); outline-offset: 1px; }
