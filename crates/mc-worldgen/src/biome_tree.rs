...
fn temp_idx(t: f64) -> usize {
    if t < -0.45 { 0 }       // frozen
    else if t < -0.15 { 1 }  // cold
    else if t < 0.2 { 2 }    // temperate
    else if t < 0.55 { 3 }   // warm
    else { 4 }               // hot
}
...
fn near_inland(t: f64, h: f64, e: f64, pv_val: f64, w: f64) -> Biome {
    ...
    if e < -0.0375 {
        if pv_val > 0.3 { return pick_peak_biome(t, h, w); }
        return pick_slope_biome(t, h);
    }
    if e < 0.45 {
        if pv_val > 0.3 { return pick_slope_biome(t, h); }
        return pick_middle_biome(t, h, w);
    }
    if e < 0.55 {
        return pick_middle_biome(t, h, w);
...
fn far_inland(t: f64, h: f64, e: f64, pv_val: f64, w: f64) -> Biome {
    ...
    if e < 0.45 {
        if pv_val > 0.3 { return pick_plateau_biome(t, h, w); }
        return pick_middle_biome(t, h, w);
    }
    if e < 0.55 {
        if pv_val > 0.3 { return pick_badlands_or_middle(t, h, w); }
        return pick_middle_biome(t, h, w);
    }
...
fn river(t: f64) -> Biome {
    if temp_idx(t) == 0 { Biome::FrozenRiver } else { Biome::River }
}
...
fn valley_biome(t: f64, h: f64) -> Biome {
    let ti = temp_idx(t);
    if ti == 0 { return Biome::FrozenRiver; }
    if ti >= 4 {
        if humid_idx(h) >= 3 { return Biome::MangroveSwamp; }
        return Biome::Desert;
    }
    if humid_idx(h) >= 3 { return Biome::Swamp; }
    Biome::River
}
...
const MIDDLE_BIOMES_VARIANT: [[Option<Biome>; 5]; 5] = [
    ...
    [Some(Biome::SunflowerPlains), None, None, Some(Biome::OldGrowthBirchForest), None],
...
const PLATEAU_BIOMES: [[Biome; 5]; 5] = [
    ...
    [Biome::Meadow, Biome::Meadow, Biome::Meadow, Biome::Meadow, Biome::DarkForest],
    [Biome::SavannaPlat, Biome::SavannaPlat, Biome::Forest, Biome::Forest, Biome::Jungle],
...
const SLOPE_BIOMES: [[Biome; 5]; 5] = [
    [Biome::SnowySlopes, Biome::SnowySlopes, Biome::SnowySlopes, Biome::SnowySlopes, Biome::SnowySlopes],
    [Biome::SnowySlopes, Biome::SnowySlopes, Biome::Grove, Biome::Grove, Biome::Grove],
    [Biome::Meadow, Biome::Meadow, Biome::Meadow, Biome::Meadow, Biome::Meadow],
    [Biome::Savanna, Biome::Savanna, Biome::Forest, Biome::Forest, Biome::Jungle],
    [Biome::Badlands, Biome::Badlands, Biome::Badlands, Biome::WoodedBadlands, Biome::WoodedBadlands],
];
...
fn pick_peak_biome(t: f64, h: f64, w: f64) -> Biome {
    ...
    if ti >= 4 {
        // Hot peaks
        if hi < 2 { return Biome::ErodedBadlands; }
    ...
    if w > 0.0 { return Biome::JaggedPeaks; }
    Biome::FrozenPeaks
}
