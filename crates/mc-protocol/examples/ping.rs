use mc_protocol::{ping_java, resolve_address, SlpConfig};
...
async fn main() {
    ...
    match resolve_address(&address).await {
        Ok(resolved) => {
            ...
            println!("\nPinging via SLP...");
            match ping_java(&resolved.ip, resolved.port, &resolved.hostname, &SlpConfig::default()).await {
                Ok(response) => {
                    println!("  Online!");
                    println!("  Version: {} (protocol {})", response.version.name, response.version.protocol);
                    println!("  Players: {}/{}", response.players.online, response.players.max);
                    println!("  MOTD: {}", response.description.to_plain_text());
                    println!("  Latency: {}ms", response.latency_ms);
                    println!("  Favicon: {}", if response.favicon.is_some() { "yes" } else { "no" });
